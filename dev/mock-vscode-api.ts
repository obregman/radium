...
/**
 * Generates mock data handler script for Codebase Map panel
 */
export function getCodebaseMapMockDataScript(): string {
  return `
    // Mock data for Codebase Map panel
    const mockGraphData = {
      nodes: [
        { id: 'auth.ts:login', name: 'login', type: 'function', filePath: 'src/services/auth.ts', line: 10 },
        { id: 'auth.ts:logout', name: 'logout', type: 'function', filePath: 'src/services/auth.ts', line: 25 },
        { id: 'auth.ts:User', name: 'User', type: 'class', filePath: 'src/services/auth.ts', line: 1 },
        { id: 'api.ts:fetch', name: 'fetch', type: 'function', filePath: 'src/services/api.ts', line: 5 },
        { id: 'api.ts:ApiClient', name: 'ApiClient', type: 'class', filePath: 'src/services/api.ts', line: 1 },
        { id: 'helpers.ts:formatDate', name: 'formatDate', type: 'function', filePath: 'src/utils/helpers.ts', line: 1 }
      ],
      edges: [
        { source: 'auth.ts:login', target: 'auth.ts:User', type: 'uses' },
        { source: 'auth.ts:logout', target: 'auth.ts:User', type: 'uses' },
        { source: 'api.ts:ApiClient', target: 'api.ts:fetch', type: 'contains' },
        { source: 'auth.ts:login', target: 'api.ts:fetch', type: 'calls' },
        { source: 'auth.ts:login', target: 'helpers.ts:formatDate', type: 'calls' }
      ]
    };
    ...
    window.__handleWebviewMessage = function(message) {
      switch (message.type) {
        case 'ready':
          console.log('[Mock Data] Webview ready, sending codebase graph data');
          setTimeout(() => {
            window.__postMessageToWebview({ type: 'graph:data', nodes: mockGraphData.nodes, edges: mockGraphData.edges });
          }, 100);
          break;
        case 'goToSymbol':
          console.log('[Mock Data] Would go to symbol:', message.filePath, message.line);
